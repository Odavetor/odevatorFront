...
import { fetchLedger } from '@entities/user'
...
export function LedgerView() {
  ...
  const load = useCallback(async (beforeId?: number) => {
    ...
    try {
      const data = await fetchLedger({ limit: PER_PAGE, before_id: beforeId })
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col gap-1">
            <Kicker tone="rose">{tt({ ru: 'Кошелёк', en: 'Wallet', de: 'Geldbörse' })}</Kicker>
            <DisplayTitle size="md">
              {tt({ ru: 'Слоты и баланс', en: 'Slots and balance', de: 'Slots und Guthaben' })}
            </DisplayTitle>
          </div>
        </div>
      </motion.header>
