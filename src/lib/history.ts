...
import { listJobHistory } from './api/undress'
import { isMockEnabled, IS_DEV } from './dev'
import { MOCK_HISTORY } from './dev-mock'
...
// Универсальный загрузчик истории.
// В моке (dev only) — отдаёт инлайн MOCK_HISTORY.
// В обычном режиме — POST /api/v1/undress/history на Go-бэк.
export async function fetchHistory(opts: {
  ...
  try {
    const data = await listJobHistory({ limit: opts.perPage ?? 10, before_id: opts.before_id })
    return {
